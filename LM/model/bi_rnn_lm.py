...
class BiRNNLanguageModel(object):
    def __init__(self, vocabulary_size, args):
        ...
        #self.seq_len = tf.Print(self.seq_len,[self.seq_len],"self.seq_len: ")
        ...
        with tf.name_scope("bi-rnn"):
            ...
            #fw_outputs = tf.Print(fw_outputs,[fw_outputs],"fw_outputs: ")
            bw_outputs = rnn_outputs[1][:, 2:, :]
            #fw_outputs = rnn_outputs[0][:, 1:-1, :]
            #bw_outputs = rnn_outputs[1][:, 1:-1, :]
            self.fw_outputs = fw_outputs
            self.bw_outputs = bw_outputs
